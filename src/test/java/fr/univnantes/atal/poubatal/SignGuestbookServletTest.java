...
public class SignGuestbookServletTest {
  ...
  public void testDoPost() throws IOException, EntityNotFoundException {
    ...
    String poubatalName = "TestGuestbook";
    String testContent = "Test Content";

    when(request.getParameter("poubatalName")).thenReturn(poubatalName);
    ...
    verify(response).sendRedirect("/poubatal.jsp?poubatalName=TestGuestbook");

    User currentUser = UserServiceFactory.getUserService().getCurrentUser();

    Entity greeting = DatastoreServiceFactory.getDatastoreService().prepare(new Query()).asSingleEntity();

    assertEquals(poubatalName, greeting.getKey().getParent().getName());
